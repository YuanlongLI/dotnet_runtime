...
namespace System.Text.Json.Serialization.Tests
{
    ...
    public abstract partial class ConstructorTests
    {
        ...
        {
            Assert.Null(JsonSerializer.Deserialize<Point_2D>("null"));
            Assert.Null(JsonSerializer.Deserialize<Point_3D>("null"));
        }
        ...
        {
            var personClass = JsonSerializer.Deserialize<Person_Class>(Person_Class.s_json);
            personClass.Verify();

            var personStruct = JsonSerializer.Deserialize<Person_Struct>(Person_Struct.s_json);
            personStruct.Verify();
        }
        ...
        {
            Point_With_Array obj1 = JsonSerializer.Deserialize<Point_With_Array>(Point_With_Array.s_json);
            ((ITestClass)obj1).Verify();

            Point_With_Dictionary obj2 = JsonSerializer.Deserialize<Point_With_Dictionary>(Point_With_Dictionary.s_json);
            ((ITestClass)obj2).Verify();

            Point_With_Object obj3 = JsonSerializer.Deserialize<Point_With_Object>(Point_With_Object.s_json);
